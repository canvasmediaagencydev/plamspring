...
type ProjectPage = Pick<Tables<"project_pages">, "id" | "name" | "slug">;
...
export default function AdminProjectsPage() {
  ...
  const load = async () => {
    const [{ data: projectsData }, { data: pagesData }] = await Promise.all([
      supabase.from("projects").select("*").order("sort_order"),
      supabase.from("project_pages").select("id, name, slug").order("sort_order"),
    ]);
  ...
  const openEdit = (item: Project) => {
    setForm({
      ...
      linked_project_page_id: item.linked_project_page_id ?? "",
    });
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      linked_project_page_id: form.linked_project_page_id || null,
  ...
  const getLinkedPageName = (pageId: string | null) => {
    if (!pageId) return null;
    return projectPages.find((p) => p.id === pageId)?.name ?? null;
  };
  ...
  return (
    <div>
      <AdminHeader
        title="โครงการของเรา"
        description="จัดการ Card โครงการในหน้าแรก"
        action={
            ...
            return (
              ...
              >
                ...
                <div className="relative aspect-video overflow-hidden bg-gray-50">
                  ...
                  {/* Status badge */}
                  <span
                    className={`absolute right-2 top-2 rounded-full px-2.5 py-0.5 text-[10px] font-semibold shadow-sm ${
                ...
                <div className="p-4">
                  ...
                  {linkedName ? (
                    <p className="mt-1 flex items-center gap-1 truncate text-[10px] text-blue-500">
                      <Link2 size={10} />
                      {linkedName}
                    </p>
                  ) : (
      ...
      <Dialog
        ...
        title={modal?.mode === "new" ? "เพิ่มโครงการ" : "แก้ไขโครงการ"}
        size="lg"
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Palm Springs Signature"
              />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <select
              value={form.linked_project_page_id}
              onChange={(e) => set("linked_project_page_id", e.target.value)}
            ...
            >
              ...
              {projectPages.map((page) => (
                <option key={page.id} value={page.id}>
                  {page.name}
                  {page.slug ? ` (/projects/${page.slug})` : " (ยังไม่มี slug)"}
                </option>
              ))}
            </select>
            <p className="text-[11px] text-gray-400">
              เมื่อผู้ใช้คลิกการ์ดบนหน้าแรกจะไปยังหน้าโครงการที่เลือก
            </p>
          </div>
