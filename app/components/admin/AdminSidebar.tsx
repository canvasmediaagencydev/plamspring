...
import {
  Home, FileText, Users, Settings, LogOut, ChevronDown, BookOpen,
  ExternalLink, LayoutDashboard, Building2,
} from "lucide-react"
...
const NAV_ITEMS: NavItem[] = [
  ...
  {
    label: "ฟอร์มติดต่อ", icon: <FileText size={16} />,
    children: [
  ...
  { label: "Blog & CSR", href: "/admin/posts", icon: <FileText size={16} /> },
  {
    label: "หน้าแรก", icon: <Home size={16} />,
    children: [
  ...
  { label: "โครงการ", icon: <Building2 size={16} />, children: [{ label: "จัดการโครงการ", href: "/admin/projects" }] },
  {
    label: "About Us", icon: <BookOpen size={16} />,
    children: [
  ...
  { label: "Our Family", href: "/admin/our-family", icon: <Users size={16} /> },
  { label: "Footer & Contact", href: "/admin/footer", icon: <Settings size={16} /> },
]
    ...
    <aside className="flex h-full w-60 flex-col border-r border-gray-100 bg-white/80 backdrop-blur-xl shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
          ...
          {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.label}>
                  <button onClick={() => toggleExpanded(item.label)}
                    className={cn("group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold transition-all duration-300",
                      isActive ? "bg-[#09418C]/10 text-[#09418C] shadow-sm" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900")}>
                    <span className={cn("shrink-0 transition-colors duration-300", isActive ? "text-[#09418C]" : "text-gray-400 group-hover:text-gray-600")}>{item.icon}</span>
                  ...
                  <div className={cn("grid transition-all duration-300 ease-in-out", isExpanded ? "grid-rows-[1fr] opacity-100 mt-1 mb-2" : "grid-rows-[0fr] opacity-0")}>
                        ...
                        {item.children.map((child) => {
                          ...
                          return (
                            <Link key={child.href} href={child.href}
                              className={cn("block rounded-lg px-3 py-2 text-xs font-medium transition-all duration-300 relative",
                                isChildActive ? "bg-white text-[#09418C] shadow-sm ring-1 ring-gray-100/50" : "text-gray-500 hover:bg-gray-50 hover:text-gray-800")}>
                              {isChildActive && <span className="absolute -left-[14px] top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-[#09418C]"></span>}
                              {child.label}
                            </Link>
                          )
                        })}
            ...
            return (
              <Link key={item.label} href={item.href!}
                className={cn("group flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold transition-all duration-300 relative overflow-hidden",
                  isActive ? "bg-[#09418C]/10 text-[#09418C] shadow-sm" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900")}>
                {isActive && <span className="absolute left-0 top-0 h-full w-1 bg-[#09418C] rounded-r-full"></span>}
                <span className={cn("shrink-0 transition-colors duration-300", isActive ? "text-[#09418C]" : "text-gray-400 group-hover:text-gray-600")}>{item.icon}</span>
                {item.label}
              </Link>
            )
      ...
      <div className="space-y-1.5 border-t border-gray-100/50 px-4 py-4 bg-gray-50/50">
        ...
        <button onClick={handleLogout}
          ...
          <LogOut size={14} className="group-hover:text-red-500 transition-colors" />ออกจากระบบ
        </button>
      </div>
    </aside>
  )
